...
void Bureaucrat::signAForm(AForm& AForm) // Sign AForm
{
    if(grade <= AForm.getsignGrade() && !AForm.isAuthorised())
        AForm.beSigned(*this);
    else if (AForm.isAuthorised())
        std::cout << "AForm already signed.\n";
    else
        std::cout << getName() << " couldn't sign AForm because their Grade was " << getGrade() << " and it must be " << AForm.getsignGrade() << ".\n";
}
